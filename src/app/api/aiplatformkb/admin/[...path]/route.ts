...
// posture, reaffirmed by the user); the deployment firewall around the
// aiplatformkb FastAPI is the gate. If AIPLATFORMKB_ADMIN_TOKEN is set in
// server env, the proxy attaches it as `Authorization: Bearer <token>` so
// deployments that opt into the upstream bearer-token gate still work
