...
// ai-platform replica of /chat/pr-feed/[prId]. Read + sync triggers go
// against ai-platform's /kb/* surface; presentation components (cards,
// filter bar, impacts table, drawer, pagination) are reused unchanged.
...
import RunPopulateButton from "@/components/pr-sync-ai-platform/RunPopulateButton";
import RunClassifyAndPopulateButton from "@/components/pr-sync-ai-platform/RunClassifyAndPopulateButton";
import ForceClassifyButton from "@/components/pr-sync-ai-platform/ForceClassifyButton";
import ForcePopulateButton from "@/components/pr-sync-ai-platform/ForcePopulateButton";
import PopulateProgressBanner from "@/components/pr-sync-ai-platform/PopulateProgressBanner";
import { useSyncRowStatus } from "@/hooks/ai-platform/useSyncRowStatus";
...
export default function PrFeedAiPlatformDetailPage() {
  return (
...
function PageInner() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getPrDetail(prIdNum, debouncedFilters, { limit: PAGE_SIZE, offset })
    ...
    return () => { cancelled = true; };
  }, [prIdNum, debouncedFilters, offset, refreshKey]);

  useEffect(() => {
    const qs = filtersToQuery(debouncedFilters, offset);
    router.replace(`/chat/pr-feed-ai-platform/${prId}${qs}`, { scroll: false });
