...
//   4. Forwards request WITHOUT an Authorization header when AIPLATFORMKB_ADMIN_TOKEN
//      is empty (auth-less Lime → aiplatformkb posture per Phase 6 §2.5).
...
describe("admin proxy — security", () => {
  ...
  it("forwards upstream WITHOUT Authorization header when token env unset", async () => {
    ...
    // Auth-less by design (Phase 6 §2.5 posture). Request goes through.
    expect(res.status).toBe(200);
    expect(fetchMock).toHaveBeenCalledTimes(1);
